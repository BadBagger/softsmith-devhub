...
- BuildSmith `v0.1.0-devhub` was published at `https://github.com/BadBagger/buildsmith/releases/tag/v0.1.0-devhub` with `BuildSmith.apk` and `BuildSmith-release-v0.1.0-devhub.apk`; it is a local-first app-builder planning tool with project blueprints, Codex prompt generation, launch checklist, demo projects, and Icon Studio for local white-background removal/export. DevHub connection was added in `v2.1.10-buildsmith`.
