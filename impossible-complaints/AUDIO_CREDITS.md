...
As of this pass, **all music and SFX in this pack are built from Freesound.org
recordings** (CC0 or CC-BY only — no NC, ND, SA, or Sampling+), processed (trimmed,
normalized, pitch-shifted, re-spliced, and/or mixed into loops) rather than used
verbatim in most cases. **Voice lines are the one exception** — those remain 100%
synthesized placeholder TTS; Freesound is not an appropriate source for character
dialogue and none was used there. See `AUDIO_DIRECTION.md` §7 for the policy
statement this file backs up.
